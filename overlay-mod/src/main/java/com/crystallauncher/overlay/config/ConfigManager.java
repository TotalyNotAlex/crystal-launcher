...
public class ConfigManager {
    ...
    public static OverlayConfig getConfig() {
        if (config == null) config = OverlayConfig.createDefault();
        return config;
    }
    ...
    public static void save() {
        if (configPath == null) configPath = FabricLoader.getInstance().getConfigDir().resolve("crystallauncher-overlay.json");
    ...
    public static void apply(OverlayConfig newConfig) { config = newConfig; save(); }
    public static void resetToDefault() { config = OverlayConfig.createDefault(); save(); }
}