...
contextBridge.exposeInMainWorld('api', {
  ...
  getSavedSkins: () => ipcRenderer.invoke('get-saved-skins'),
  ...
  applyMicrosoftSkin: (skinPath, variant, accountId) => ipcRenderer.invoke('apply-microsoft-skin', { skinPath, variant, accountId }),
  ...
  pingServer: (address, port) => ipcRenderer.invoke('ping-server', { address, port }),
  analyzeCrash: (content) => ipcRenderer.invoke('analyze-crash', content),
  modrinthInstallWithDeps: (url, profileId, fileName, versionId) => ipcRenderer.invoke('modrinth-install-mod-with-deps', { url, profileId, fileName, versionId }),
  listScreenshots: () => ipcRenderer.invoke('list-screenshots'),
  readScreenshot: (name) => ipcRenderer.invoke('read-screenshot', name),
  deleteScreenshot: (name) => ipcRenderer.invoke('delete-screenshot', name),
  openScreenshotsFolder: () => ipcRenderer.invoke('open-screenshots-folder'),
  backupWorld: (id) => ipcRenderer.invoke('backup-world', id),
  exportProfile: (id) => ipcRenderer.invoke('export-profile', id),
  importProfile: () => ipcRenderer.invoke('import-profile'),
  importModpackFile: (kind) => ipcRenderer.invoke('import-modpack-file', kind),
  downloadUpdateBg: (url) => ipcRenderer.invoke('download-update-bg', url),
  installBgUpdate: () => ipcRenderer.invoke('install-bg-update'),
  taskbarProgress: (pct) => ipcRenderer.invoke('taskbar-progress', pct),
  onDeepLink: (cb) => ipcRenderer.on('deep-link', (e, d) => cb(d)),
});
